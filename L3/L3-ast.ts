...
const isGoodFields = (fields: Sexp): fields is string[] => isArray(fields) && allT(isString, fields);

export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    if (!isGoodBindings(methods) || !isGoodFields(fields)) {
    ...
    // After isGoodBindings, bindings has type [string, Sexp][]
    const methodVars = map(b => b[0], methods);
    const valsResult = mapResult(parseL3CExp, map(second, methods));
    const bindingsResult = mapv(valsResult, (vals: CExp[]) => zipWith(makeBinding, methodVars, vals));
...
export const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    ...
    const names = map(b => b[0] as string, methods1);
    const values = mapResult(parseL3CExp, map(second, methods1));
...
const unparseClassExp = (ce: ClassExp): string => {
    const fieldsStr = ce.fields.map((field: VarDecl) => field.var).join(" ");
    const methodsStr = ce.methods.map((method: Binding) => 
        `(${method.var.var} ${unparseL3(method.val)})`).join(" ");
    return `(class (${fieldsStr}) (${methodsStr}))`;
}
